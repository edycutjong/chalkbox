...
test.describe("create flow", () => {
  ...
  test("in-scope prompt with no live sim shows an honest coming-soon state, not the wrong sim", async ({
    page,
  }) => {
    await page.goto("/");

    // Physics is in scope, but this demo only ships the fraction-division sim.
    await page
      .getByPlaceholder(/Show why dividing by a fraction/i)
      .fill("Show why a heavy and a light ball fall at the same rate.");

    await page.getByRole("button", { name: /create manipulative/i }).click();

    // Honest state: the real safety verdict + a flagship CTA — never a manipulative
    // that doesn't match the prompt's subject, and never a published result.
    const card = page.getByTestId("unsupported-card");
    await expect(card).toBeVisible({ timeout: 10_000 });
    await expect(card.getByText(/subject=physics/i)).toBeVisible();
